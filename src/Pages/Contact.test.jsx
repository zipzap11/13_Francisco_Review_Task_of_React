...
describe("Contact", () => {
  ...
  test("Fill input field with valid value", () => {
    ...
    fireEvent.input(screen.getByTestId("fullname"), {
      target: { value: "Francisco" },
    });
    fireEvent.input(screen.getByTestId("email"), {
      target: { value: "Francisco@gmail.com" },
    });
    ...
    expect(screen.getByTestId("fullname")).toHaveValue("Francisco");
    expect(screen.getByTestId("email")).toHaveValue("Francisco@gmail.com");
    expect(screen.getByTestId("phonenumber")).toHaveValue("098123789");
    expect(screen.getByTestId("select")).toHaveValue("Indonesia");
    expect(screen.getByTestId("message")).toHaveValue("This is test message.");
  });
  ...
  test("Fill input field with invalid value", () => {
    ...
    fireEvent.input(screen.getByTestId("fullname"), {
      target: { value: "Franci10" },
    });
    fireEvent.input(screen.getByTestId("email"), {
      target: { value: "Francgmail.com" },
    });
    fireEvent.input(screen.getByTestId("phonenumber"), {
      target: { value: "09812hr" },
    });
    const errFullName =
